...
describe('promo', () => {
  it('finds promo codes case-insensitively', () => {
    expect(findPromoCode('super10', SHOW_CONFIG)?.code).toBe('SUPER10')
  })

  it('rejects unknown promo codes', () => {
    const result = validatePromoCode('INVALID', ['A5'], SHOW_CONFIG)
  ...
  it('rejects promo when minimum order value is not met', () => {
    const result = validatePromoCode('SUPER10', ['F4'], SHOW_CONFIG)
    expect(result).toEqual({
      valid: false,
  ...
  it('accepts percentage promo when minimum order value is met', () => {
    const result = validatePromoCode('SUPER10', ['A5'], SHOW_CONFIG)
  ...
  it('rejects VIP promo when not all seats are VIP', () => {
    const result = validatePromoCode('VIPFLAT100', ['A5', 'C2'], SHOW_CONFIG)
    expect(result).toEqual({
      valid: false,
  ...
  it('accepts flat VIP promo when all conditions are met', () => {
    const result = validatePromoCode('VIPFLAT100', ['A5', 'B6'], SHOW_CONFIG)
    expect(result.valid).toBe(true)
    if ('promo' in result) {
  ...
  it('accepts FIRST5 on the first order', () => {
    const result = validatePromoCode('FIRST5', ['A5'], SHOW_CONFIG, {
  ...
  it('accepts PREMIUM20 when no General seats are selected', () => {
    const result = validatePromoCode('PREMIUM20', ['A5', 'C2'], SHOW_CONFIG)
    expect(result.valid).toBe(true)
    if ('promo' in result) {
  ...
  it('rejects PREMIUM20 when a General seat is selected', () => {
    const result = validatePromoCode('PREMIUM20', ['A5', 'F4'], SHOW_CONFIG)
    expect(result).toEqual({
      valid: false,
